...
test("renderExplorationHtml renders state screenshots, issues, and edges", () => {
  ...
  assert.doesNotMatch(html, /Copy ignore entry/);
  assert.doesNotMatch(html, /copy-ignore-entry/);
  assert.doesNotMatch(html, /Copied ignore entry/);
  ...
  assert.doesNotMatch(html, /class="report-view-controls"/);
  assert.doesNotMatch(html, /data-report-view="full"/);
  assert.doesNotMatch(html, /data-finding-filter="all"/);
  assert.doesNotMatch(html, /data-finding-filter-status/);
  assert.doesNotMatch(html, /Showing ' \+ visibleStates \+ ' state/);
  assert.doesNotMatch(html, /a11y-shiftleft:report-view:/);
  assert.doesNotMatch(html, /a11y-shiftleft:finding-filter:/);
  assert.doesNotMatch(html, /data-state-critical=/);
  assert.doesNotMatch(html, /data-state-no-findings=/);
  assert.doesNotMatch(html, /data-issue-critical=/);
  assert.doesNotMatch(html, /data-issue-warning=/);
  assert.doesNotMatch(html, /data-issue-info-only=/);
...
test("renderExplorationHtml shows the first ten affected findings for very large groups", () => {
  const repeatedIssues = Array.from({ length: 31 }, (_, index) => ({
    ...issues[0],
    severity: "warning" as const,
    selector: `.repeated-button-${index + 1}`,
    fingerprint: `button-name::state-1::repeated-large-${index + 1}`
  }));
  const html = renderExplorationHtml(graph, repeatedIssues);
  const issueHtml = issueBlockForRule(html, "button-name");
...
test("writeExplorationHtml can create a unified audit report", async () => {
  ...
  assert.match(html, /Review record fields: per-step status, task outcome, first blocker, blocker severity, missing states, task evidence attachments, remediation owner, retest date, and evidence links\./);
  ...
  assert.match(html, /inclusive constraint, outcome, blocker, owner, and retest date/);
  assert.match(html, /temporary acceptances/);
  assert.match(html, /expiring soon/);
