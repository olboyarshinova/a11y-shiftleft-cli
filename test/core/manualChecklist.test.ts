...
test("createManualChecklist generates human-review checklist items", () => {
  ...
  assert.equal(checklist.items.some((item) => item.id === "images-of-text-review"), true);
  ...
  assert.equal(checklist.items.some((item) => item.id === "non-text-contrast-review"), true);
  ...
  assert.equal(checklist.items.some((item) => item.id === "character-shortcuts-review"), true);
  ...
  assert.equal(checklist.items.some((item) => item.id === "motion-actuation-review"), true);
...
test("toManualChecklistMarkdown renders actionable Markdown checkboxes", () => {
  ...
  assert.match(markdown, /Not reviewed \| 30/);
  assert.match(markdown, /Multiple ways to find important pages/);
  assert.match(markdown, /Images of text have a text alternative or exception/);
  assert.match(markdown, /Character key shortcuts can be turned off or remapped/);
  assert.match(markdown, /Motion actuation has non-motion alternatives/);
  assert.match(markdown, /Non-text contrast for controls and graphics/);
