...
export function formatAuditDeviceMatrixSummary(results: Array<{ target: AuditDeviceTarget; failed: boolean; outputDir: string }>): string {
  const rows = results.map((result) => (
    `| ${escapeMarkdownTableCell(result.target.label)} | ${result.failed ? "failed" : "completed"} | [Open report](${escapeMarkdownLink(`${result.outputDir}/a11y-report.html`)}) |`
  )).join("\n");
