...
function formatCoverageMatrix(report: A11yReport): string {
  ...
  const embeddedFindings = report.issues.filter((issue) => issue.source === "embedded-content" || issue.ownership?.kind === "third-party-embed").length;
  const voiceControlSignals = report.issues.filter((issue) => (
...
| Reflow at 400% (320 CSS px simulation) | ${reflowCount > 0 ? "Heuristic evidence collected" : "Not included"} | ${reflowCount} rendered state${reflowCount === 1 ? "" : "s"} checked for overflow, clipped text, and fixed or sticky overlap |
