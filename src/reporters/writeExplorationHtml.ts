...
function renderCoverageMatrix(
  ...
  const rows = [
    ...
    coverageRow("reflow", "Reflow at 400% (320 CSS px simulation)", reflowStates.length > 0 ? evidenceState(countIssues((issue) => issue.source === "layout")) : "not-tested", reflowStates.length > 0 ? "Automated evidence" : "Review required", reflowStates.length > 0 ? `${reflowStates.length} state${reflowStates.length === 1 ? "" : "s"} checked for overflow and clipped text` : "This audit did not collect reflow evidence; run a reflow check to verify overflow and clipped text.", reflowStates.length > 0, reflowStates.length > 0 ? countIssues((issue) => issue.source === "layout") : undefined, manualChecklistItemIds),
