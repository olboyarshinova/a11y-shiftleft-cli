...
- Added critical, warning, and info counts to the `evidence export` journey
  terminal summary.
- Added the `evidence-summary.md` review path to `evidence verify` output.
...
- Added bounded pointer-interaction inventory to exploration evidence, visual
  Audit Coverage, Markdown reports, and the manual checklist so sliders, range
