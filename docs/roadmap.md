...
- Continue improving per-criterion coverage rows with status, evidence source,
  ...
  WCAG Review Coverage table, and the visual report now shows a compact
  WCAG Review Coverage panel. Next, keep the full JSON matrix complete while
  validating the visual wording in real reviews.
- Continue extending reflow and resize-text evidence beyond the current 200%
  zoom and 400% / 320 CSS px simulation. Current evidence includes horizontal
  overflow, clipped text, bounded fixed/sticky overlap candidates, and compact
  200% vs 400% comparison rows; next, validate speed and false-positive wording
  in real responsive layouts.
- Continue cross-browser evidence beyond the current `--browser` and
  ...
  overlay sliders, lightweight PNG pixel-difference metadata, compact
  pixel-diff meters, screenshot-difference hotspots, review-priority badges,
  and reproduction signals for manual triage; next, refine heatmap-style
  image-diff visualization for larger screenshots.
  Clearly label
...
- Continue responsive web evidence beyond the current `--mobile`, `--tablet`,
  ...
  pixel-difference metadata, compact pixel-diff meters,
  screenshot-difference hotspots, review-priority badges, and reproduction
  signals for manual triage; next, refine heatmap-style image-diff
  visualization for larger screenshots.
  Treat this as responsive/mobile browser testing for rendered websites, not as
  native iOS or Android app auditing.
