...
- Continue cross-browser evidence beyond the current `--browser` and
  ...
  page, UI-state, shared-state-difference, visual-evidence-link, and
  reproduction signals for manual triage; next, add screenshot-level visual
  comparison. Clearly label browser-specific differences for manual review
  before treating them as product defects.
- Continue responsive web evidence beyond the current `--mobile`, `--tablet`,
  ...
  visual-evidence-link, and reproduction signals for manual triage; next, add
  screenshot-level visual comparison. Treat this as
