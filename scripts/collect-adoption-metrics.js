...
export async function collectAdoptionMetrics(options = {}, deps = {}) {
...
export async function writeAdoptionMetrics(outputPath, metrics) {
  await fs.mkdir(path.dirname(outputPath), { recursive: true });
  await fs.writeFile(outputPath, `${JSON.stringify(metrics, null, 2)}\n`);
}

async function fetchNpmDownloads(fetchImpl, packageName, period) {
...
async function fetchNpmRegistryMetadata(fetchImpl, packageName) {
...
async function fetchGitHubTraffic(fetchImpl, repo, githubToken) {
  if (!githubToken) {
    return {
      available: false,
